 ...
 *
 * Relocated from `shared/commonTest/VaryKeysBehaviorTest.kt` (design decision #5, Engram #1505).
 */
@OptIn(InternalPersistentCacheApi::class)
abstract class VaryKeysConformanceTest<P> : CacheConformanceSupport<P>() {

    private lateinit var storage: FileCacheStorage<P>
    ...
    @Test
    fun `given same url with different varyKeys when stored then creates separate cache entries`() =
        runTest {
            // Given
    ...
    @Test
    fun `given cached entries with different varyKeys when finding with specific varyKeys then returns correct entry`() =
        runTest {
            // Given
            ...
            storage.store(
                url,
                TestDataFactory.createCachedResponseWithVaryKeys(
                    url = url.toString(),
    ...
    @Test
    fun `given cached entry with varyKeys when finding with different varyKeys then returns null`() =
        runTest {
            // Given
            val url = Url("https://api.example.com/data")
    ...
    @Test
    fun `given cached entry without varyKeys when finding with empty varyKeys then returns entry`() =
        runTest {
            // Given
            val url = Url("https://api.example.com/data")
    ...
    @Test
    fun `given multiple cached entries with different varyKeys when findAll then returns all entries for url`() =
        runTest {
            // Given
            val url = Url("https://api.example.com/data")
            storage.store(
    ...
    @Test
    fun `given entries for different urls when findAll then returns only entries for specified url`() =
        runTest {
            // Given
    ...
    @Test
    fun `given cached entries with different varyKeys when removing specific varyKeys then only that entry is removed`() =
        runTest {
            // Given
            val url = Url("https://api.example.com/data")
            val varyKeysEnglish = mapOf("Accept-Language" to "en")
    ...
    @Test
    fun `given cached entries with different varyKeys when removeAll then all entries for url are removed`() =
        runTest {
            // Given
            val url = Url("https://api.example.com/data")
            storage.store(
    ...
    @Test
    fun `given entries for multiple urls when removeAll for one url then other url entries remain`() =
        runTest {
            // Given
            val url1 = Url("https://api.example.com/users")
            val url2 = Url("https://api.example.com/posts")
    ...
    @Test
    fun `given entry with multiple varyKeys when finding with same keys then returns entry`() =
        runTest {
            // Given
            val url = Url("https://api.example.com/data")
            val varyKeys = mapOf(
    ...
    @Test
    fun `given entry with multiple varyKeys when finding with partial keys then returns null`() =
        runTest {
            // Given
            val url = Url("https://api.example.com/data")
    ...
    @Test
    fun `given varyKeys in different order when stored and retrieved then matches correctly`() =
        runTest {
            // Given
            val url = Url("https://api.example.com/data")
