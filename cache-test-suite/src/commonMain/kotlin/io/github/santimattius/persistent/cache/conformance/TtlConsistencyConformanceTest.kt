 ...
 *
 * Relocated from `shared/commonTest/TtlConsistencyBehaviorTest.kt` (design decision #5,
 * Engram #1505).
 */
@OptIn(InternalPersistentCacheApi::class)
abstract class TtlConsistencyConformanceTest<P> : CacheConformanceSupport<P>() {
    ...
    fun `given non-expired entries exceeding size limit when cleanup runs then oldest entries removed`() =
        runTest {
            ...
            val clock = TestClock()
            val storage = newStorage(maxSize = 500, ttl = 60 * 60 * 1000, clock = clock::now)
    ...
    fun `given corrupted cache file when cleanup runs then corrupted file is removed`() = runTest {
        ...
        fileSystem.write(
            cacheFilePath("corrupted_0.cache"),
            "not valid protobuf data".encodeToByteArray()
        )
