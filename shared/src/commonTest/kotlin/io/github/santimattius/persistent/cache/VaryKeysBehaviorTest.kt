...
class VaryKeysBehaviorTest {
    ...
    fun `given cached entry without varyKeys when finding with empty varyKeys then returns entry`() =
        runTest {
            ...
            val url = Url("https://api.example.com/data")
            val response =
                TestDataFactory.createCachedResponse(url = url.toString(), body = "No vary keys")
    ...
    fun `given entries for different urls when findAll then returns only entries for specified url`() =
        runTest {
            ...
            storage.store(
                url1, TestDataFactory.createCachedResponseWithVaryKeys(
                    url = url1.toString(),
                    body = "Users EN",
                    varyKeys = mapOf("Accept-Language" to "en")
                )
            ...
            storage.store(
                url2, TestDataFactory.createCachedResponse(
                    url = url2.toString(),
                    body = "Posts"
                )
            )
    ...
    fun `given entries for multiple urls when removeAll for one url then other url entries remain`() =
        runTest {
            ...
            storage.store(
                url2, TestDataFactory.createCachedResponseWithVaryKeys(
                    url = url2.toString(),
                    body = "Posts EN",
                    varyKeys = mapOf("Accept-Language" to "en")
                )
