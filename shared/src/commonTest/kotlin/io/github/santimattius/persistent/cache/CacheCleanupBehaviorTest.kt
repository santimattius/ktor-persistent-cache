...
class CacheCleanupBehaviorTest {
    ...
    @Test
    fun `given store operation when cleanup is triggered then operation completes without deadlock`() = runTest {
    ...
    @Test
    fun `given multiple rapid store operations when cleanup runs each time then all complete`() = runTest {
        // Given
        val config = OkioFileCacheConfig(
            fileName = "http_cache",
    ...
    fun `given cache with maxSize zero when store is called then cleanup is skipped`() = runTest {
        ...
        assertTrue(
            cacheFiles.size == 5,
            "All 5 cache files should exist when cleanup is disabled"
        )
    }

    @Test
    fun `given cache with negative maxSize when store is called then cleanup is skipped`() = runTest {
    ...
    @Test
    fun `given interleaved store and find operations when cleanup runs then operations remain consistent`() = runTest {
        // Given
        val config = OkioFileCacheConfig(
            fileName = "http_cache",
            maxSize = 500,
